    ...
    <div className="animate-in fade-in duration-500">
      <div className="ph">
        <div>
          ...
          <p>Controle de exclusividade, prazos e splits contratuais</p>
        </div>
      ...
      <div className="admin-card">
        <div className="overflow-x-auto">
          <table className="admin-table">
            <thead>
              <tr>
                <th>ID</th><th>Artista / Parte</th><th>Tipo de Acordo</th><th>Excl.</th><th>Split (A/L)</th><th>Vigência</th><th>Vencimento</th><th>Status</th><th>Doc</th>
              </tr>
            </thead>
            <tbody>
              {contracts.map((c, i) => (
                <tr key={i}>
                  ...
                  <td className="font-black italic uppercase tracking-tighter">{c[1]}</td>
                  <td className="text-[11px] font-black uppercase text-admin-muted">{c[2]}</td>
                  <td><span className={`admin-badge ${c[3]==='Sim'?'badge-green':'badge-gold'}`}>{c[3]}</span></td>
                  <td className="font-mono text-[11px] font-bold">{c[4]}</td>
                  <td className="text-admin-muted font-bold text-[11px]">{c[5]}</td>
                  <td className="text-admin-text font-black text-[11px]">{c[6]}</td>
