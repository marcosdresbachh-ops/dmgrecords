    ...
    <div class="g2">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="form-group">
            <label class="form-label">ID da Carteira (Asaas Wallet)</label>
            <input class="form-input" value="bbcb3f26-1c3e-43e3-941b-ebdbf829d1f0" readonly style="background:var(--bg3);cursor:not-allowed;border-style:dashed;">
            <div class="form-hint">Seu identificador único para recebimentos na plataforma.</div>
          </div>
...
async function doRegister(){
  ...
  const whatsapp = document.getElementById('reg-wpp').value.trim();
  const bizName = document.getElementById('reg-biz').value.trim();
  const email = document.getElementById('reg-email').value.trim();
  const password = document.getElementById('reg-pass').value;
  const pass2 = document.getElementById('reg-pass2').value;

  if(!email||!password||!name||!bizName||!whatsapp){toast('Preencha todos os campos obrigatórios','warn');return;}
  if(password!==pass2){toast('As senhas não coincidem','err');return;}
  if(password.length<6){toast('A senha deve ter no mínimo 6 caracteres','warn');return;}
  ...
  try {
    ...
    if (!res.ok) {
      const { error } = await res.json();
      throw new Error(error || 'Ocorreu um erro no cadastro.');
    }
...
if (typeof initPainel !== 'undefined') {
...
export default function ValeIndicaPainelPage() {
    ...
    useEffect(() => {
        ...
        script.id = scriptId;
        script.innerHTML = getPainelScript();
