...
export function PageHero({ eyebrow, title, description }: PageHeroProps) {
  return (
    <div className="bg-foreground relative overflow-hidden px-6 py-11 md:px-12 md:py-14">
      ...
      <div className="relative z-[2] mx-auto max-w-[1280px]">
        <div className="mb-3.5 flex items-center gap-2.5 font-['DM_Mono',monospace] text-[.6rem] uppercase tracking-[.28em] text-white/45 before:block before:h-px before:w-5 before:bg-primary">
          {eyebrow}
        </div>
        <h1 className="mb-3.5 font-['Playfair_Display',serif] text-[clamp(2.4rem,5vw,4rem)] font-black leading-[.97] text-white">
          {title}
        </h1>
        <p className="max-w-[500px] text-[.95rem] leading-[1.75] text-white/60">{description}</p>
      </div>
    </div>
  );
}
