  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex justify-between items-end">
        <div>
          <h1 className="text-4xl font-black italic uppercase tracking-tighter text-zinc-900 leading-none">Analytics & SoundCloud Insights</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-white border border-zinc-200 p-6 rounded-2xl group hover:border-primary/20 transition-all shadow-sm">
            <div className="p-2 w-fit bg-zinc-50 border border-zinc-100 rounded-lg text-primary mb-4 group-hover:scale-110 transition-transform">{s.icon}</div>
            <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-1">{s.l}</p>
            <p className="text-2xl font-black text-zinc-900 italic tracking-tighter">{s.v}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white border border-zinc-200 rounded-[32px] p-8 space-y-8 shadow-sm">
          ...
          <div className="h-[300px] flex items-end gap-4 px-4">
            {[40, 65, 45, 90, 55, 75, 100, 85].map((h, i) => (
              <div key={i} className="flex-1 flex flex-col gap-2 items-center group">
                <div className="w-full bg-[#ff5500]/10 rounded-t-lg group-hover:bg-[#ff5500]/20 transition-all" style={{ height: `${h}%` }} />
                <div className="w-full bg-primary/10 rounded-t-lg group-hover:bg-primary/20 transition-all" style={{ height: `${h * 0.7}%` }} />
                <span className="text-[8px] font-black text-zinc-400 uppercase">W{i+1}</span>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white border border-zinc-200 rounded-[32px] p-8 shadow-sm">
          ...
          <div className="mt-10 p-4 bg-primary/5 border border-primary/10 rounded-2xl">
